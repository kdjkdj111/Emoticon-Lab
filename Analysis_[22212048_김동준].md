...
| :--- | :--- | :--- | :--- |
| 05/08/2026 | 1.0 | First Writing | Kim dong jun |
| 05/31/2026 | 1.1 | Usecase & Domain Analysis fix | Kim dong jun |
</div>
<br>
...
### 1) Use case diagram
<img src="resource/UseCaseDiagram.png" width="100%" alt="Logo">
본 다이어그램은 Emoticon-Lab 시스템을 이용하는 주체(Actor)들과 시스템이 제공하는 핵심 기능(Use Case) 간의 상호작용을 구조화한 도식입니다.<br><br>
...
| :--- | :--- |
| **Step** | **Action** |
| S | 사용자가 기술 분석 결과에서 에러가 발생한 특정 이미지만 교체하여 문제를 해결한다. |
| 1 | 이 Use case는 사용자가 기술 분석 상세 리포트 화면에서 특정 이미지의 '현재이미지 개별 교체' 버튼을 누를 때 시작된다. |
| 2 | 시스템은 사용자에게 파일 선택기를 띄운다. |
| 3 | 사용자가 수정한 새로운 이미지를 선택한다. |
| 4 | 시스템은 선택된 이미지를 기존 슬롯에 덮어쓰고 전체 업로드 이미지 상태를 갱신한다. |
| 5 | 시스템은 해당 슬롯의 에러를 '해결됨(Resolved)'으로 처리하고 리포트 데이터를 저장한다. |
| 6 | 시스템은 갱신된 에러 리스트를 화면에 보여주고 성공 알림 메시지를 출력한다. |
| 7 | 이 Use case는 이미지 교체 및 에러 상태 갱신이 완료되면 끝난다. |

| EXTENSION SCENARIOS | |
| :--- | :--- |
| **Step** | **Branching Action** |
| 4 | 4a. 새로 선택한 파일의 규격이 맞지 않을 시 실패한다.<br> 4a1. 교체 실패 메시지를 출력하고 교체 과정을 중단한다. |
...
| 4 | 시스템은 프로젝트명, 분석 일시, 썸네일 등의 요약 데이터를 프로젝트 카드 리스트 형태로 렌더링한다. |
| 5 | 사용자가 특정 프로젝트 카드를 클릭하면, 시스템은 해당 프로젝트의 상세 데이터를 로드하여 워크스페이스 화면으로 이동한다. |

| EXTENSION SCENARIOS | |
| :--- | :--- |
| **Step** | **Branching Action** |
| 3 | 3a. DB 연결 오류로 인해 데이터를 불러올 수 없다.<br> 3a1. "데이터를 불러오는 중 오류가 발생했습니다" 메시지를 출력한다. |
| 4 | 4a. 해당 사용자가 생성한 프로젝트 내역이 존재하지 않는다. (리스트가 비어있음)<br> 4a1. "내역이 없습니다" 메시지를 출력한다. |
| 5 | 5a. 사용자가 기존 프로젝트를 클릭하지 않고 '새 프로젝트 시작' 버튼을 클릭한다.<br> 5a1. 업로드 화면으로 이동한다. |
...
| 구분 | 클래스명 | 설명 |
| :--- | :--- | :--- |
| **1. Entity Class** | 1) User | 사용자의 정보와 계정 상태를 관리하는 클래스이다. (ex: id, email, nickname, apiUsageCount) |
| | 2) Project | 작업 세션 정보를 저장하는 클래스이다. (ex: id, userId, title, status, createdAt) |
| | 3) EmoticonImage | 이미지 파일의 메타데이터를 보관하는 클래스이다. (ex: id, projectId, slotIndex, imageUrl) |
| | 4) TechnicalReport | 기술 검수 결과 데이터를 보관하는 클래스이다. (ex: id, projectId, strayPixelCount, marginErrorList) |
| | 5) AIReport | AI 분석 결과 데이터를 보관하는 클래스이다. (ex: id, emotionKeywords, consistencyScore, feedbackText) |
| | 6) Pixel | 검출된 개별 에러 픽셀의 상세 좌표 정보를 보관하는 클래스이다. (ex: id, coordinates) |
| **2. Control class** | 7) AuthControl | 사용자의 로그인, 회원가입, 로그아웃 등 인증 흐름을 제어하는 클래스이다. (ex: signIn(), signUp(), signOut(), getSession()) |
| | 8) WorkspaceControl | 프로젝트 생성, 삭제 및 목록 조회, 상태 업데이트 등 워크스페이스의 데이터 작업 흐름을 제어하는 클래스이다. (ex: getUserProjects(), createProject(), updateProject(), deleteProject()) |
| | 9) StorageControl | 이미지 파일 업로드, 저장 및 경로 반환을 처리하는 클래스이다. (ex: uploadImages(), getImagePath()) |
| | 10) TechnicalAnalysisControl | 이미지 처리 알고리즘을 통해 스트레이 픽셀, 해상도, 여백 규격 등을 정밀 검수하는 클래스이다. (ex: requestAnalysis(), scanPixels(), detectErrors()) |
| | 11) AIAnalysisControl | 외부 AI를 통해 디자인 일관성 점수, 감정 키워드, 개선 피드백을 도출하는 클래스이다. (ex: requestAIAnalysis(), calculateConsistency(), generateAIReport()) |
| **3. Boundary Class<br>(View)** | 12) StartView | 시작 페이지, 회원가입, 로그인 화면을 제공하고 인증 흐름의 진입점 역할을 하는 클래스이다. |
| | 13) DashboardView | 사용자의 과거 작업 프로젝트 목록을 렌더링하고 프로젝트 입장 및 신규 생성을 관리하는 클래스이다. |
| | 14) UploadView | 이미지를 입력받아 서버로 전송하고 업로드 현황을 표시하는 클래스이다. |
| | 15) WorkspaceView | 메인 작업실 컨테이너로서 탭 네비게이션을 통해 하위 뷰들을 관리하고 프로젝트 데이터를 보유하는 클래스이다. |
| | 16) SimulatorView | 모의 채팅창 UI를 제공하고 테마 변경 및 이모티콘 발송을 시뮬레이션하는 클래스이다. |
| | 17) TechnicalView | 기술 분석 결과를 시각화하고 오류 이미지의 개별 교체 기능을 제공하는 클래스이다. |
| | 18) AIView | AI 분석 옵션(타겟 연령/타입)을 입력받고, 분석 결과를 차트와 텍스트로 시각화하는 클래스이다. |
...
**1) StartView**: <br>
<img src="resource/prototype/1.png" width="100%"><br>
: 첫 시작화면이다. 테스트 시작하기를 누르면 로그인 화면으로 넘어간다.<br>
<img src="resource/prototype/2.png" width="100%"><br>
: 로그인 화면이다. ID와 비밀번호를 입력하고 로그인할 수 있으며 계정이 없을 시 회원가입을 진행한다.<br>
<img src="resource/prototype/3.png" width="100%"><br>
: 회원가입은 닉네임, 이메일, 비밀번호로 진행된다.<br>
<img src="resource/prototype/4.png" width="100%"><br>
: 로그인을 통해 인증 성공 시 작업실로 이동 할 수 있다.<br>
<br> **2) DashboardView**:<br>
<img src="resource/prototype/5.png" width="100%"><br>
: 과거 프로젝트 목록이 나열되고, 각 프로젝트 선택시 과거 작업을 이어갈 수 있다. 새 프로젝트 시작 시 업로드 창으로 넘어간다.<br>
<img src="resource/prototype/6.png" width="100%"><br>
: 과거 프로젝트에 재진입한 시점이다.
<br> **3) UploadView**:<br>
<img src="resource/prototype/7.png" width="100%"><br>
: 업로드 화면이다. 해당 부분 클릭하면 파일 선택기가 열리며 드래그 앤 드롭으로 최대 32종의 이미지를 업로드 할 수 있다.<br>
<img src="resource/prototype/8.png" width="100%"><br>
: 32종의 이미지를 업로드한 상태이다. 이미지 사이즈, 파일 형식 등 기본적인 규격을 점검 후에 워크스페이스로 이동할 수 있다.<br>
<br> **4) Simulator View**:<br>
<img src="resource/prototype/9.png" width="100%"><br>
: 워크스페이스 초기 화면이다. 좌측 상단에서 프로젝트의 제목을 설정할 수 있다.<br>
<img src="resource/prototype/10.png" width="100%"><br>
: 시뮬레이션 뷰에선 업로드한 이미지를 Send와 Receiver 입장에서 사용해보고 받아볼 수 있다.<br>
추가로 가운데 인터페이스를 통해 테마를 커스터마이징하여 이모티콘의 가독성도 직접 확인할 수 있다.<br>
<br> **5) TechnicalView**:<br>
<img src="resource/prototype/11.png" width="100%"><br>
: 기술 분석 탭이다. 분석 시작 버튼을 통해 이미지를 서버로 보내고 픽셀 단위의 분석을 할 수 있다.<br>
<img src="resource/prototype/12.png" width="100%"><br>
: 분석 데이터를 리턴받을 때까지 로딩을 출력한다.<br>
<img src="resource/prototype/13.png" width="100%"><br>
: 기술 분석 결과 Error가 있는 이미지의 id와 분석 내용을 출력하고, 좌표를 받아 이미지 위에 표시한다.<br>
Error를 수정했다면 해당 이미지만 교체하고 현재 프로젝트에 반영할 수 있다.<br>
<br> **6) AlView**:<br>
<img src="resource/prototype/14.png" width="100%"><br>
: Al server로 분석 요청을 보내기 전에 현재 프로젝트의 상세 정보를 사용자가 입력할 수 있다.<br>
연령층, 타입, 설명을 통해 분석에 영향을 줄 수 있다.<br>
<img src="resource/prototype/15.png" width="100%"><br>
: 분석 데이터를 리턴받을 때까지 로딩을 출력한다.<br>
<img src="resource/prototype/16.png" width="100%"><br>
: AI 분석 결과 디자인 일관성, 감정 스펙트럼, 가독성, 트렌드 반영성, 종합 평가를 제공한다.<br>
감정 스펙트럼은 이모티콘 세트의 균형을 시각적으로 표현하기 위해 방사형 그래프를 사용하였다.<br>
<br> **7) NavigationBar**:<br>
<img src="resource/prototype/17.png" width="100%"><br>
: 워크스페이스의 네비게이션 바에서 초기화를 누를 시 현재 프로젝트를 초기화하는 팝업이 뜨며 초기화하고 업로드뷰로 돌아간다.<br>
<img src="resource/prototype/18.png" width="100%"><br>
: 워크스페이스의 네비게이션 바에서 세션 종료를 누르면 현재 프로젝트가 저장되고 작업실에 반영된다.<br>
