...
| **3. Boundary Class<br>(View)** | 12) StartView | 랜딩 페이지, 회원가입, 로그인 화면을 제공하고 인증 흐름의 진입점 역할을 하는 클래스이다. |
| | 13) DashboardView | 사용자의 과거 작업 프로젝트 목록을 렌더링하고 프로젝트 입장 및 신규 생성을 관리하는 클래스이다. |
| | 14) UploadView | 32종의 이미지를 드래그 앤 드롭으로 입력받아 서버로 전송하고 업로드 현황을 표시하는 클래스이다. |
| | 15) WorkspaceView | 메인 작업실 컨테이너로서 탭 네비게이션을 통해 하위 뷰들을 관리하고 프로젝트 데이터를 보유하는 클래스이다. |
| | 16) SimulatorView | 발신자와 수신자 양방향의 모의 채팅창 UI를 제공하고 테마 변경 및 이모티콘 발송을 시뮬레이션하는 클래스이다. |
| | 17) TechnicalView | 기술 분석 결과를 시각화하고 오류 이미지의 개별 교체 기능을 제공하는 클래스이다. |
| | 18) AIView | AI 분석 옵션(타겟 연령/타입)을 입력받고, 분석 결과를 차트와 피드백 텍스트로 시각화하는 클래스이다. |
